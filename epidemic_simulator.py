...
class Person:
    def __init__(self, x, y):
        self.x = x
        self.y = y
        self.dx = randint(-1, 1)  # diferenciax
        self.dy = randint(-1, 1)
        self.illness = 0
        self.immune = 0
        self.mask = False
...
def draw(people):
    ...
    font = pygame.font.SysFont("Calibri", 30)  # font typ,font size
    ...
    text = font.render(
        "Clear:      Infected:       Immune:        Mask:", True, (200, 200, 200)
    )
    textRect = text.get_rect()
    textRect.left = 700
    textRect.top = 10
